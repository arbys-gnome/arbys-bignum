...
cmp_abs_range(const big_int& a, size_t a_start, size_t a_len,
              const big_int& b, size_t b_len) noexcept {
    if (a_len != b_len) {
        return a_len <=> b_len;
    }
...
static void sub_abs_inplace(std::vector<limb_t>& a, const size_t start,
                            const big_int& b, const size_t b_len) noexcept {
    const auto& b_limbs = big_int_access::limbs(b);
    dlimb_t borrow = 0;
...
[[nodiscard]] static limb_t shift_left(std::vector<limb_t>& vec,
                                        const size_t len,
...
static void shift_right(std::vector<limb_t>& vec,
                        const size_t len,
                        const unsigned shift) noexcept {
    if (shift == 0) return;
...
[[nodiscard]] static std::expected<DivisionResult, errors::ArithmeticError>
div_single_limb(const big_int& dividend, const limb_t divisor) noexcept {
...
// Knuth's Algorithm D
[[nodiscard]] static std::expected<DivisionResult, errors::ArithmeticError>
div_multi_limb(const big_int& dividend, const big_int& divisor) noexcept {
    const size_t m = big_int_access::length(dividend);
